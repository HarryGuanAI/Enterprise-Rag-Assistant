...
项目将 API Key、管理员密码、JWT 密钥放在环境变量中，前端不会接触真实 Key。公开运行时使用游客限额控制调用成本，用严格拒答避免无依据调用 LLM。部署时数据库和后端容器端口只绑定服务器本机地址，公网入口交给 Nginx 反向代理。公开仓库前应重新生成并废弃开发阶段暴露过的 Key，确保仓库历史里没有真实密钥。
