...
## 2026-05-13 文档沉淀：Embedding、Hybrid Search 与 Query 处理

### 已完成
- 将“当前 Embedding 是否为稠密模型、向量维度是多少”的答案整理进 README、维护手册、面试复盘和求职项目讲法。
- 明确记录当前使用 DashScope `text-embedding-v4`，输出 1024 维稠密向量，数据库字段为 pgvector `vector(1024)`。
- 将 Hybrid Search 原理整理为可公开解释的版本：pgvector 向量召回 + 中文关键词召回 + 融合排序 + 轻量 Rerank。
- 将 Query 处理策略整理成面试问答：当前不做 LLM Query Rewrite，但会做能力咨询识别、多轮追问判断、文本归一化、关键词抽取和当前问题支持度校验。
- 记录“薪酬绩效/绩效薪酬”词序变化导致误拒答的原因、修复方式和面试讲法。
- 新增 `docs/resume-project-notes.md`，集中放简历项目描述、1 分钟/3 分钟讲法和高频追问，避免把求职内部话术混入开源 README。

### 验证记录
- 文档更新未读取或展示 `.env` 中的真实密钥。
- README 保持开源项目口径，不写“求职”“面试”“演示项目”等内部定位。

## 2026-05-13 域名接入准备与简历提示词

### 已完成
- 记录域名 `airagcloud.online` 的接入计划：DNS A 记录目标为京东云公网 IP `117.72.45.27`，当前备案中。
- 服务器 Nginx 已提前配置 `airagcloud.online`、`www.airagcloud.online` 和 `117.72.45.27` 作为 `server_name`。
- 维护文档中补充备案通过后的待办：DNS 验证、HTTPS 证书、`.env` 域名切换、前端重新构建和线上验证。
- 求职项目讲法文档中新增“给简历优化 AI 的提示词”，用于后续在新窗口完善个人简历。

### 待办
- 备案通过后检查公共 DNS 是否返回 `117.72.45.27`。
- 配置 Let's Encrypt / Certbot HTTPS。
- 将公开访问地址从 IP 切换到 `https://airagcloud.online`。
